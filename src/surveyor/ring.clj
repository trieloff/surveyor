;;dirty fork from https://github.com/sudharsh/clj-oauth2/blob/52e731c4cb4aa4807c53ef8ad442684d6dbf4a04/src/clj_oauth2/ring.clj
...
(defn is-callback [request oauth2-params]
  ...
  (let [oauth2-url-vector (string/split (.toString (java.net.URI. (:redirect-uri oauth2-params))) #"\?")
        ...
        nope (pprint/pprint request)
        nope (println (:uri request) " = " oauth2-uri (request-uri request oauth2-params) (= oauth2-uri (request-uri request oauth2-params)))
        nope (println (:uri request) (submap? (keyify-params (parse-params oauth2-url-params encoding)) (:params request)))]
...
(defn wrap-oauth2
  ...
  (fn [request]
    (println "oauth-wrapper for " (:uri request))
    (if (excluded? (:uri request) oauth2-params)
      ...
      (if (is-callback request oauth2-params)
        ...
        (let [nope (println "is-callback: true")
              response {:status 302
